...
F_corechains = open("data/lcquad_test_corechain.txt", "w")
F_error = open("data/lcquad_test_corechain_error.txt", "w")
F_terminal = open("data/lcquad_test_terminal.txt", "w")
F_time = open("data/lcquad_test_time.txt", "w")
F_has_no_ans = open("data/lcquad_test_has_no_ans.txt", "w")

F_error_lcquad_query = open("data/lcquad_test_runningquery_error_not_imp.txt", "w")

lcquad2_temp = lcquad_templates()
lcquad_MUE = cache_lcquad_entities()

#--------- put cache corechain in a list (written to save time)
F_corechains_from_cache = open("data/lcquad_cache/lcquad_cache_corechain.txt", "r")
...
def corechain_product_list_itself(entityIds_arr, oneHop_arr, answerCC, qUID, tempID, lcquadQues, dataType, answerFound):
    ...
    for cc_lbl in prod_lists_labels:
        ...
        line = ', '.join(entityIds_arr).replace('wd:', '') + "\t" + ccSign + "\t" + cc_lbl + "\t" + prod_lists_ids[i]
...
def fix_and_report_answerNotFound(entityIds_arr, answerCC, qUID, tempID, lcquadQues, dataType, lcquadQues_Query):
    ...
    if not systemIssue:
        ...
        cc_line = ', '.join(entityIds_arr).replace('wd:', '') + "\t" + ccSign + "\t" + ' '.join(cc_lbls) + "\t" + ' '.join(cc_ids)
        write_to_file(F_corechains, cc_line) 
...
def corechain_from_cache(entityIds_arr, answerCC, qUID, tempID, lcquadQues, dataType, lcquadQues_Query):
    ...
    for ccLine in cacheCorechain:
        ...
        if entityIds == ccCache_entIds:
            ...
            cc_line = ', '.join(entityIds_arr).replace('wd:', '') + "\t" + ccCache_sign + "\t" + ccCache_lbl + "\t" + ccCache_id
            write_to_file(F_corechains, cc_line)
...
def lcquad_test_corechain():
    F_most_used_entities = open("data/lcquad2_dataset/lcquad_test_answer.txt", "r")
    ...
    for line in out:
        ...
        if not found_in_cache:
            if i > 0 : #  if entityIds_arr == ['wd:Q183257', 'wd:Q2275640']: # 28113
...
#* Result for all test (cache is not excluded)
# The answer is FOUND :) 
# Max time 60.704309940338135
# Min time 0.3336489200592041
# Avg time 0.5044269074554588
# All Time 17704.077476978302
